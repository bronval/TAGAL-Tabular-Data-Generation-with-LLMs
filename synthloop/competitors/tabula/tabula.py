...
from competitors.tabula.tabula_dataset import TabulaDataset, TabulaDataCollator
from competitors.tabula.tabula_start import TabulaStart, CategoricalStart, ContinuousStart, RandomStart
from competitors.tabula.tabula_trainer import TabulaTrainer
from competitors.tabula.tabula_utils import _array_to_dataframe, _get_column_distribution, _convert_tokens_to_text, \
...
class Tabula:
    ...
    #         # skip column if only nan produced
    #         if data[self.label_encoder_list[i]['column']].isnull().all():
    #             print(f"skip column: {self.label_encoder_list[i]['column']}")
    #             continue
    ...
    #         # data.loc[:, self.label_encoder_list[i]['column']] = pd.to_numeric(data[self.label_encoder_list[i]['column']], errors='coerce')
    ...
    #         # data.loc[:, self.label_encoder_list[i]['column']] = data[self.label_encoder_list[i]['column']].astype(float)
    ...
    def decode_categorical_column(self, data: pd.DataFrame):
        ...
        for i in range(len(self.label_encoder_list)):
            # skip column if only nan produced
            print(data[self.label_encoder_list[i]['column']])
            if data[self.label_encoder_list[i]['column']].isnull().all():
                print(f"skip column: {self.label_encoder_list[i]['column']}")
                print("--------------------")
                continue
            ...
            print(f"column: {self.label_encoder_list[i]['column']}")
    ...
    def sample(self, n_samples: int,
        ...
        if self.categorical_columns == []:
            ...
            print(f"shape: {df_inversed.shape}")
            return df_inversed
